...
class Robot:
    ...
    def kinematics(self,dt):
        ...
        if self.heading>2*math.pi or self.heading <- 2*math.pi:
            self.vr = max(min(self.maxapeed,self.vr),self.minspeed)
            sefl.vl = max(min(self.maxspeed,self.vl),self.minspeed)

class Graphics:
    def __init__(self,dimenions,robot_img_path,map_img_path):
        pygame.init()
        self.black = (0,0,0)
        ...
        self.height, self.width = dimenions
